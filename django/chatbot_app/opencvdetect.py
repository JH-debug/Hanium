...
img_rm = cv2.GaussianBlur(img, (3, 3), 0)
img_gray = cv2.cvtColor(img_rm, cv2.COLOR_BGR2GRAY)
...
def get_text_from_image(image):
    ...
    import io
    os.environ['GOOGLE_APPLICATION_CREDENTIALS']=r"C:\Users\User\Desktop\visionAPI-a21221d148d7.json"
    ...
    #im_b64 = base64.b64encode(image)

    client = vision.ImageAnnotatorClient()
    textlist = ''
    cv2.imwrite('cv2test.jpg', image)
    with io.open('./cv2test.jpg', 'rb') as image_file:
...
def main():
    ...
    print_configs()
    image_path = './ocrtest.jpg'  # todo parameter 로 image file path 를 입력받도록 하자.
